...
function emitirCertificado(nome) {
  ...
  const numeroChamadas = 4;

  if (
    contadorDeChamadas < numeroChamadas &&
    contadorDeEmissoesDeCertificado == 0
  ) {
    let mensagemTimeout;
    ...
    if (contadorDeChamadas == 1) {
      ...
      mensagemTimeout = `Boa! Mas você precisa chamar essa função 4 vezes.\nVocê já chamou a função ${contadorDeChamadas} ${
    ...
    nome = nome === "" || nome === undefined ? "[nome está vazio]" : nome;
    ...
    if (
      ...
      const totalChamadasLoopInfinito =
        contadorDeEmissoesDeCertificado * numeroChamadas + contadorDeChamadas;
      alert(
